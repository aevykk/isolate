...
#+END_SRC
Create the template dataset as described below (=Creating the backing Linux system root=), copy the static seccomp wrapper into the template's PATH from =${pkgs.isolate}/bin/seccomp_wrapper= (see step 4 below), then populate clones by (re)starting the unit:
#+BEGIN_SRC
